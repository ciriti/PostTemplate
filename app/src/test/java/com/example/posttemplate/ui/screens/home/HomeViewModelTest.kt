...
class HomeViewModelTest {
    ...
    fun `load posts intent updates state and emits no effect on success`() = testScope.runTest {
        turbineScope {
            ...
            assertEquals(
                HomeState(isLoading = false, posts = mockPosts, errorMessage = null),
                states.awaitItem()
            )
    ...
    fun `load posts intent updates state and emits effect on error`() = testScope.runTest {
        turbineScope {
            ...
            // Assert
            assertEquals(
                HomeState(isLoading = false, posts = emptyList(), errorMessage = null),
                states.awaitItem()
            )
            assertEquals(
                HomeState(isLoading = true, posts = emptyList(), errorMessage = null),
                states.awaitItem()
            )
            assertEquals(
                HomeState(isLoading = false, posts = emptyList(), errorMessage = errorMessage),
                states.awaitItem()
            )
